...
Evaluate generated images along **object, color, shape, texture, count and spatial relation** using fixed Grounding DINO and Qwen3-VL rules. The generation backbone is not part of the evaluator: provide images and structured targets from STAR, another autoregressive model, a diffusion model or any other image source.

These are **exploratory automated metrics**, not validated human accuracy. The code separates visual observation from reference scoring. Models do not receive the expected answer, baseline ID or experiment metadata.

## Evaluator assignment

| Semantic | Tool | Score |
|---|---|---|
| Object | Grounding DINO | Presence/absence match |
| Color | Qwen3-VL | Exterior-color category match |
| Shape | Qwen3-VL | Overall-outline category match |
| Texture | Qwen3-VL | Pattern or surface-roughness match |
| Count | Grounding DINO | Exact count; also deviation from requested count |
| Spatial relation | DINO for 2D; Qwen for depth/containment | Relation category match |

**[Complete rules and default parameters](docs/evaluators.md)** · **[Input/output contract](docs/interface.md)** · **[Origin and limitations](docs/provenance.md)**
